...
def playGame(quickMove, token, show=False):
  ...
  while True:
    ...
    #print(f"{brd=}")
    #print("tkn: "+tknToPlay)
    #print(findMoves(brd, tknToPlay))
    #list(map(lambda x: {y[0] * 8 + y[1] for y in x}, findMoves(brd, tknToPlay)))
    moveTmp = findMoves(brd,tknToPlay)
    moves = {k[0]*8+k[1] for k in moveTmp}
    if not (moves):
...
# Parse args here
tkn, trnyCt, defFile = "x", 1, "othello6"    # token, game count, users file name
for arg in args:
